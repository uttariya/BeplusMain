...
namespace beplusService.Controllers
{
    public class BepBloodRequestController : TableController<BepBloodRequest>
    {
        ...
        {
            var count = context.BepBloodRequests.Where(x => x.RecipientPhone == item.RecipientPhone).Count();
            if (count > 0)
            {
                return BadRequest("Phone number already registered!");
            }
            count = context.BepBloodRequests.Where(x => x.RecipientEmail == item.RecipientEmail).Count();
            if (count > 0)
            {
